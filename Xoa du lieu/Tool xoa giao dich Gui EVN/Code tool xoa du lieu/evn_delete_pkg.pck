create or replace package evn_delete_pkg is
  ...
  */
  ...
  procedure delete_ap_invoice_row(p_rowid            varchar2,
...
create or replace package body evn_delete_pkg is
  ...
  */
  ...
  begin
    --#20170212.TamPT5.Security by User       
    if not evn_security_pkg.security_request_delete then
      return;
    end if;
    --End#20170212
    ...
    for r in c_invoice loop
      ...
      select evn_delete_request_log_s.nextval into v_line_id from dual;
      insert into evn_delete_request_log
      values
        (v_line_id,
         fnd_global.conc_request_id,
         'EVN AP Delete Unvalidated Invoice',
         'p_company_code: ' || p_company_code || '; p_invoice_id: ' ||
         p_invoice_id || '(invoice_num: ' || r.invoice_num || ')',
         (select user_name
            from fnd_user u
  ...
  begin
    ...
    for r in c_invoice loop
      ...
      values
        (v_line_id,
         fnd_global.conc_request_id,
         'EVN AP Delete Cancelled Invoice',
         'p_company_code: ' || p_company_code || '; p_invoice_id: ' ||
         p_invoice_id || '(invoice_num: ' || r.invoice_num || ')',
         (select user_name
            from fnd_user u
  ...
  begin
    ...
    for r in c_check loop
      ...
      values
        (v_line_id,
         fnd_global.conc_request_id,
         'EVN AP Delete Voided Payment',
         'p_company_code: ' || p_company_code || '; p_check_id: ' ||
         p_check_id || '(check_number: ' || r.check_number || ')',
         (select user_name
            from fnd_user u
           where u.user_id = fnd_global.user_id),
         sysdate);
  ...
  begin
    ...
    for r in c_trans loop
      ...
      values
        (v_line_id,
         fnd_global.conc_request_id,
         'EVN AR Delete draft transaction',
  ...
  begin
    ...
    for rs in c_receipt loop
      ...
      values
        (v_line_id,
         fnd_global.conc_request_id,
         'EVN AR Delete Reversed Receipt ',
         'p_company_code: ' || p_company_code || '; p_receipt_id: ' ||
         p_receipt_id || '(receipt_number: ' || rs.receipt_number || ')',
         (select user_name
            from fnd_user u
           where u.user_id = fnd_global.user_id),
         sysdate);
  ...
  begin
    ...
    --delete_ap_invoice_row(v_row_id, 'APXINWKB');
  ...
  begin
    ...
    insert into backup.evn_ar_receivable_appl_b3
      select *
        from ar_receivable_applications_all ar
       where ar.cash_receipt_id = p_receipt_id;
  
    insert into backup.evn_ar_trx_summary_hist_b3
      select *
        from ar_trx_summary_hist ats
       where ats.cash_receipt_id = p_receipt_id;
  
    insert into backup.evn_ar_misc_cash_dist_all_b3
      select *
        from ar_misc_cash_distributions_all mcd
       where mcd.cash_receipt_id = p_receipt_id;
    ...
    delete from ar_misc_cash_distributions_all mcd
     where mcd.cash_receipt_id = p_receipt_id;
  ...
  begin
    ...
    --
    current_calling_sequence := 'evn_delete_pkg.delete_ap_invoice_row<-' ||
                                p_calling_sequence;
    ...
    select invoice_id, invoice_type_lookup_code
      ...
      from ap_invoices_all --PhongDV2 ap_invoices => ap_invoices_all
     where rowid = p_rowid;
  
    -- Delete attachments
    debug_info := 'Delete from fnd_attachments';
    fnd_attached_documents2_pkg.delete_attachments('AP_INVOICES',
                                                   l_invoice_id);
    ...
    delete from ap_invoice_distributions_all
     where invoice_id = l_invoice_id
    returning invoice_distribution_id bulk collect into l_key_value_list;
  
    --Bugfix:4670908
    debug_info := 'Delete from child entity - ap_invoice_lines';
    delete from ap_invoice_lines_all where invoice_id = l_invoice_id;
  
    --added back in the following deletes
    debug_info := 'Delete from child entity - ap_payment_schedules';
    delete from ap_payment_schedules_all where invoice_id = l_invoice_id;
  
    debug_info := 'Delete from child entity - ap_holds';
    delete from ap_holds_all where invoice_id = l_invoice_id;
    ...
    l_success := ap_etax_utility_pkg.get_event_class_code(p_invoice_type_lookup_code => l_invoice_type_lookup_code,
    ...
    l_success := ap_etax_utility_pkg.get_event_type_code(p_event_class_code         => l_event_class_code,
    ...
    zx_api_pub.global_document_update(p_api_version      => 1.0, --in parameter
    ...
    delete from ap_invoices_all where rowid = p_rowid;
  ...
  exception
    when others then
      if (sqlcode <> -20001) then
        ...
        fnd_message.set_token('PARAMETERS', 'p_Rowid = ' || p_rowid);
